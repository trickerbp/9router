...
describe("Cursor AgentService codec (cursorProtobuf.js)", () => {
  ...
  describe("McpArgs decode", () => {
    ...
    it("falls back to raw JSON bytes used by older AgentService builds", () => {
      const rawEntry = encodeField(2, LEN, Buffer.concat([
        Buffer.from(encodeField(1, LEN, "count")),
        Buffer.from(encodeField(2, LEN, "42")),
      ]));
      const decoded = decodeMcpArgs(Buffer.concat([
        Buffer.from(encodeField(1, LEN, "legacy")),
        Buffer.from(rawEntry),
      ]));
      expect(decoded.args).toEqual({ count: 42 });
    });
  });
...
describe("Cursor AgentService executor helpers (cursor.js)", () => {
  ...
  describe("buildAgentRunFrame", () => {
    ...
    it("encodes a text-only run request with system + model", () => {
      ...
      expect(run.has(8)).toBe(false); // custom_system_prompt can produce an empty turn
      ...
      expect(Buffer.from(userMessage.get(1)[0].value).toString("utf8")).toContain("be brief");
    });
  ...
  describe("executeAgent", () => {
    function installSession(executor, chunks) {
      const writes = [];
      let index = 0;
    ...
    function serverFrame(field, payload) {
      return envelope(0, encodeField(field, LEN, payload));
    }
    ...
    it("returns MCP exec requests as non-streaming OpenAI tool calls", async () => {
      const mcpArgs = Buffer.concat([
        Buffer.from(encodeField(1, LEN, "get_weather")),
        entry("city", "Hanoi"),
        Buffer.from(encodeField(3, LEN, "call_weather")),
        Buffer.from(encodeField(5, LEN, "get_weather")),
      ]);
      const execRequest = Buffer.concat([
        Buffer.from(encodeField(1, VARINT, 23)),
        Buffer.from(encodeField(11, LEN, mcpArgs)),
      ]);
      const executor = new CursorExecutor();
      installSession(executor, [serverFrame(2, execRequest)]);
    ...
    it("emits streaming tool calls with a tool_calls finish reason", async () => {
      const mcpArgs = Buffer.concat([
        Buffer.from(encodeField(1, LEN, "search")),
        entry("query", "9router"),
        Buffer.from(encodeField(3, LEN, "call_search")),
        Buffer.from(encodeField(5, LEN, "search")),
      ]);
      const execRequest = Buffer.concat([
        Buffer.from(encodeField(1, VARINT, 24)),
        Buffer.from(encodeField(11, LEN, mcpArgs)),
      ]);
      const executor = new CursorExecutor();
      installSession(executor, [serverFrame(2, execRequest)]);
      ...
      expect(sse).toContain('"tool_calls":[{"index":0,"id":"call_search"');
      expect(sse).toContain('"finish_reason":"tool_calls"');
      expect(sse).toContain("data: [DONE]");
    });

    it("surfaces Connect terminal errors instead of returning an empty success", async () => {
      const executor = new CursorExecutor();
      installSession(executor, [envelope(2, JSON.stringify({
        error: { code: "resource_exhausted", message: "Cursor quota exhausted" },
      }))]);
      ...
      expect(body.error.message).toBe("Cursor quota exhausted");
    });

    it("acknowledges request context, heartbeat, and KV before completing text", async () => {
      const requestContext = Buffer.concat([
        Buffer.from(encodeField(1, VARINT, 7)),
        Buffer.from(encodeField(10, LEN, new Uint8Array())),
      ]);
      const heartbeat = encodeField(13, LEN, new Uint8Array());
      const kvRequest = Buffer.concat([
        Buffer.from(encodeField(1, VARINT, 9)),
        Buffer.from(encodeField(2, LEN, new Uint8Array())),
      ]);
      const textDelta = encodeField(1, LEN, encodeField(1, LEN, "OK"));
      const turnEnded = encodeField(14, LEN, new Uint8Array());
      const executor = new CursorExecutor();
      const writes = installSession(executor, [Buffer.concat([
        serverFrame(2, requestContext),
        serverFrame(1, heartbeat),
        serverFrame(4, kvRequest),
        serverFrame(1, textDelta),
        serverFrame(1, turnEnded),
      ])]);
      ...
      const result = await executor.executeAgent({
        ...
        body: {
          ...
          tools: [{ function: { name: "noop", parameters: { type: "object" } } }],
        },
        stream: false,
        credentials,
      });
      const body = await result.response.json();
      const replies = writes.slice(1).map((frame) => decodeMessage(frame.subarray(5)));

      expect(body.choices[0].message.content).toBe("OK");
      expect(replies.some((message) => message.has(7))).toBe(true);
      expect(replies.some((message) => message.has(3))).toBe(true);
      const execReply = replies.find((message) => message.has(2));
      const execClient = decodeMessage(execReply.get(2)[0].value);
      expect(execClient.get(1)[0].value).toBe(7);
      expect(execClient.has(10)).toBe(true);
      const contextResult = decodeMessage(execClient.get(10)[0].value);
      const contextSuccess = decodeMessage(contextResult.get(1)[0].value);
      const decodedRequestContext = decodeMessage(contextSuccess.get(1)[0].value);
      expect(decodedRequestContext.has(7)).toBe(true);
    });
  });
});
