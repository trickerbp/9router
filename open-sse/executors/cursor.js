...
function textFromMessage(message) {
  return [textFromContent(message?.content), toolCallsFromMessage(message)]
    .filter(Boolean)
    .join("\n");
}
...
export function buildAgentRunFrame(messages, model, tools = []) {
  ...
  const history = chatMessages
    ...
    .filter(Boolean);
  const userText = [system, textFromMessage(current)]
    .filter(Boolean)
    .join("\n\n") || "Continue.";
...
export function createRequestContextResponse(execRequest, tools = []) {
  // AgentService asks every run for client context. 9router has no IDE file
  // context, but current servers expect the MCP catalog in RequestContext too.
  const requestId = typeof execRequest === "number"
    ? execRequest
    : Number(execRequest?.get(1)?.[0]?.value || 0);
  const requestContext = concatBuffers(
    ...tools.map((tool) => agentMessage(7, encodeMcpToolDefinition(tool)))
  );
  ...
  const execClientMessage = concatBuffers(
    encodeField(1, PROTOBUF_VARINT, requestId),
    agentMessage(10, requestContextResult),
  );
  return wrapConnectRPCFrame(agentMessage(2, execClientMessage));
}

export function createClientHeartbeatResponse() {
  return wrapConnectRPCFrame(agentMessage(7, new Uint8Array()));
}

export function createKvResponse(kvMessage) {
  const requestId = Number(kvMessage?.get(1)?.[0]?.value || 0);
  const resultField = kvMessage?.has(2) ? 2 : kvMessage?.has(3) ? 3 : null;
  if (!resultField) return null;
  const kvClientMessage = concatBuffers(
    encodeField(1, PROTOBUF_VARINT, requestId),
    agentMessage(resultField, new Uint8Array()),
  );
  return wrapConnectRPCFrame(agentMessage(3, kvClientMessage));
}
...
export class CursorExecutor extends BaseExecutor {
  ...
  openAgentHttp2Stream(url, headers, signal) {
    ...
    req.on("trailers", (trailers) => {
      ...
      if (grpcStatus !== "0") {
        const message = decodeURIComponent(String(trailers?.["grpc-message"] || "Cursor AgentService failed"));
        fail(new Error(`Cursor AgentService gRPC ${grpcStatus}: ${message}`));
      }
    });
    req.on("end", () => {
      ended = true;
  ...
  async executeAgent({ model, body, stream, credentials, signal }) {
        ...
        while (!finished) {
          ...
          pending = decodeAgentFrames(pending, (payload) => {
            ...
            if (serverMessage.has(1)) {
              ...
              if (update.has(13)) {
                session.write(createClientHeartbeatResponse());
              }
            ...
            if (serverMessage.has(2)) {
              ...
              if (execRequest.has(10)) {
                ...
                const name = mcp.toolName || mcp.name || "tool";
                finished = true;
                onEvent({
                  type: "tool_call",
                  value: {
                    ...
                    type: "function",
                    function: { name, arguments: JSON.stringify(mcp.args || {}) },
                  },
                });
                onEvent({ type: "done" });
              } else {
            ...
            if (serverMessage.has(4)) {
              const kvResponse = createKvResponse(decodeMessage(serverMessage.get(4)[0].value));
              if (kvResponse) session.write(kvResponse);
            }
          }, (payload) => {
            try {
              const trailer = JSON.parse(Buffer.from(payload).toString("utf8"));
              const message = trailer?.error?.message || trailer?.error?.code;
              if (!message) return;
              finished = true;
              onEvent({ type: "error", value: String(message) });
              onEvent({ type: "done" });
            } catch {}
          });
        }
    ...
    const responseStream = new ReadableStream({
      start(controller) {
        consume((event) => {
          if (event.type === "text") {
            const delta = emittedRole
              ? { content: event.value }
              : { role: "assistant", content: event.value };
            emittedRole = true;
            controller.enqueue(encoder.encode(chatChunkSse({ id: responseId, created, model, delta })));
