...
async function fetchCursorCatalog(credentials, signal, http2Post = http2PostProto) {
  ...
  const response = await http2Post(url, headers, new Uint8Array(), signal, FETCH_TIMEOUT_MS);
...
export async function resolveCursorModels(credentials, options = {}) {
  ...
  try {
    const models = await fetchCursorCatalog(credentials, options.signal, options.http2Post || http2PostProto);
