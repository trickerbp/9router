  ...
  if (useDailySummary) {
    ...
    for (const dr of dayRows) {
      ...
      for (const [akKey, ak] of Object.entries(day.byApiKey || {})) {
        ...
        const displayAkKey = apiKeyMasked ? `${apiKeyMasked}|${rawModel}|${provider || "unknown"}` : "local-no-key";
        if (!stats.byApiKey[displayAkKey]) {
          stats.byApiKey[displayAkKey] = { requests: 0, promptTokens: 0, completionTokens: 0, cost: 0, rawModel, provider: providerDisplayName, apiKeyMasked, keyName, apiKeyKey, lastUsed: dateKey };
        }
        stats.byApiKey[displayAkKey].requests += ak.requests || 0;
        stats.byApiKey[displayAkKey].promptTokens += ak.promptTokens || 0;
        stats.byApiKey[displayAkKey].completionTokens += ak.completionTokens || 0;
        stats.byApiKey[displayAkKey].cost += ak.cost || 0;
        if (dateKey > (stats.byApiKey[displayAkKey].lastUsed || "")) stats.byApiKey[displayAkKey].lastUsed = dateKey;
      }
